...
scheme = evaluationScheme(allData[1:100], method="split",train=0.8, goodRating = 4)
algorithms <- list(
   "popular items" = list(name="POPULAR", param=NULL),
   "user-based CF Cosine center nn50" = list(name="UBCF", param=list(method="Cosine", normalize='center', nn=50, minRating=5)),
   "user-based CF Pearson center nn50" = list(name="UBCF", param=list(method="pearson", normalize='center', nn=50, minRating=5)), 
   "user-based CF jaccard center nn50" = list(name="UBCF", param=list(method="jaccard", normalize='center', nn=50, minRating=5)),   
   "user-based CF jaccard center nn50" = list(name="UBCF", param=list(method="jaccard", normalize='center', nn=50, minRating=5)),   
   "user-based CF Pearson z-index nn50" = list(name="UBCF", param=list(method="pearson", normalize='Z-score', nn=50, minRating=5)),
   ...
   "item-based CF pearson center" = list(name="IBCF", param=(list(k = 5, method="pearson", normalize = "center",  alpha = 0.5))),   	
	"item-based CF karypis center" = list(name="IBCF", param=(list(k = 5, method="karypis", normalize = "center",  alpha = 0.5))),
	...
	"item-based CF karypis Z-score" = list(name="IBCF", param=(list(k = 4, method="karypis", normalize = "Z-score",  alpha = 0.5))),
